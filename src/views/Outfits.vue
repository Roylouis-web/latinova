...
import { collection, getDocs, where, query, orderBy, and, startAfter, endBefore, limit, limitToLast, type QuerySnapshot, type DocumentData } from 'firebase/firestore';
...
const nextStatus = async () => {
    ...
    const docRef = query(collection(db, 'outfits'), and(where('tags', '==', 'Normal'), where('category', '==', state.value)), orderBy('timestamp', 'desc'), startAfter(last), limit(10));
...
const prevStatus = async () => {
    ...
    const docRef = query(collection(db, 'outfits'), and(where('tags', '==', 'Normal'), where('category', '==', state.value)), orderBy('timestamp', 'desc'), endBefore(first), limitToLast(10));
    querySnapshot2.value = await getDocs(docRef);
    if (!querySnapshot2.value.empty) {
       return true;
    }
...
const fetchData = async (next: boolean, prev: boolean) => {
    ...
    if (router.currentRoute.value.params.category) {
        if (next) {
            ...
            const docRef = query(collection(db, 'outfits'), and(where('tags', '==', 'Normal'), where('category', '==', state.value)), orderBy('timestamp', 'desc'), limit(10));
            querySnapshot.value = await getDocs(docRef);
            if (!querySnapshot.value.empty) {
                querySnapshot.value.forEach((outfit) => {
                    temp.push({ id: outfit.id, ...outfit.data() } as Outfit);
...
<template>
    ...
    <section v-else-if="outfits.length" class="grow mt-20 mb-12" id="top">
        <ul
            class="flex flex-col gap-10 justify-center items-center md:grid md:grid-cols-3 md:gap-x-24 mx-auto md:w-[85%]">
