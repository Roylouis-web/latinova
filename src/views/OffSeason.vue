...
import { collection, getDocs, where, query, startAfter, endBefore, limit, orderBy, type QuerySnapshot, type DocumentData, limitToLast } from 'firebase/firestore';
...
const nextStatus = async () => {
    ...
    const docRef = query(collection(db, 'outfits'), where('tags', '==', 'Off Season'), orderBy('timestamp', 'desc'), startAfter(last), limit(10));
...
const prevStatus = async () => {
    ...
    const docRef = query(collection(db, 'outfits'), where('tags', '==', 'Off Season'), orderBy('timestamp', 'desc'), endBefore(first), limitToLast(10));
    querySnapshot2.value = await getDocs(docRef);
    if (!querySnapshot2.value.empty) {
        return true
    }
...
const fetchData = async (next: boolean, prev: boolean) => {
    ...
    if (next) {
        ...
        const docRef = query(collection(db, 'outfits'), where('tags', '==', 'Off Season'), orderBy('timestamp', 'desc'), limit(10));
        querySnapshot.value = await getDocs(docRef);
        if (!querySnapshot.value.empty) {
