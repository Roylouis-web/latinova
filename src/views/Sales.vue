...
const fetchData = async (next: boolean, prev: boolean) => {
    ...
    if (next) {
        ...
        const docRef = query(collection(db, 'orders'), where('status', '==', 'delivered'), orderBy('timestamp', 'desc'), limit(10));
        querySnapshot.value = await getDocs(docRef);
        if (!querySnapshot.value.empty) {
            querySnapshot.value.forEach((order) => sales.value.push({ id: order.id, ...order.data() } as Order));
...
<template>
    ...
    <section v-else-if="sales.length" class="grow mt-10 text-xl md:text-3xl mb-16 rounded-xl p-4">
        <ul class="flex flex-col gap-6">
            <li v-for="({ id, status, product, createdAt, deliveredAt }) in sales" :key="id">
